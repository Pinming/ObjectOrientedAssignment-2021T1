...
public class FushiSystem {
	...
	public static void main(String[] args) throws IOException {
		...
		FushiSystem app = new FushiSystem();
	...
	private StudentCatalog loadStudent() {
		...
		for (Student student : studentCatalog.students) { // 原来是 Student student : studentCatalog，使用 iter
	...
	private void run() throws IOException {
		...
		while (choice != 0) {
			StudentsFormatter sf = null;
			if (choice == 1) {
				sf = formatterFactory.getFormatter("PlainText");
			} else if (choice == 2) {
				sf = formatterFactory.getFormatter("HTML");
			} else if (choice == 3) {
				sf = formatterFactory.getFormatter("XML");
			}
			displayStudents(sf);
			choice = getChoice();
		}
	 ...
	 */
	private void displayStudents(StudentsFormatter sf){
		stdOut.println(
				sf.formatStudents(studentCatalog));
	}
